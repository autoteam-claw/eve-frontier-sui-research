...
| Assembly 类型 | 功能 | Hook 接口 | 能力边界 |
|---|---|---|---|
| Smart Gate | 星门/通行控制 | `canJump(char_id, from, to) → bool` | **仅返回布尔值**，不能内联执行支付或状态变更 |
| Smart Storage Unit | 仓库/交易所 | 自定义存取逻辑 + Custom dApp URL | 库存管理、物品交换、F 键打开 dApp |
| Smart Turret | 炮塔/防御 | 自定义打击目标逻辑 | 可编程瞄准规则，最多 3 个/Assembly |

- 所有 Assembly 依赖 **Network Node** 供能，Network Node 需持续注入燃料 → **燃料是核心稀缺资源**
- 支持自定义货币（Carbon Platform 创建部落/辛迪加代币）、去中心化市场、服务经济
- Sui 上使用 **Smart Object Framework (SOF)** 管理实体/关系/Hook（替代 EVM 上的 MUD 框架）
- 所有 Assembly 逻辑以 Sui Move 合约形式部署，可与 world-contracts 直接组合
- **链上代币体系**：EVE（区块链代币）· LUX（游戏内货币）· SUI（Gas）· Fuel（消耗品，链上可转让性待确认）
- **已有基础设施**：World API (REST/Swagger)、KillMail 链上事件、Primordium 索引器、zkLogin（CCP 已集成）、Sponsored Tx（CCP 代付 Gas）
...
| **zkLogin** | Google/Apple → Sui 钱包 | ⚠️ CCP 已将 zkLogin 集成到玩家登录流程，第三方重复建设价值低 |
| **Sponsored Tx** | Gas 代付 | CCP 已为玩家直接操作代付 Gas，第三方 Agent 需自行代付 |
...
> **可行性**：🟡 可行但需调整 — Smart Gate `canJump()` hook 只返回 bool，不能内联收费

**核心概念**：EVE Frontier 首个链上星门通行费协议，把 Smart Gate 通行费变成可配置的链上收入流。

**技术限制与应对**：
- ⚠️ `canJump(character_id, from_gate_id, to_gate_id)` **只返回 true/false**，不能在 hook 内执行支付
- ✅ **两步收费模式**：玩家先在 TollRegistry 合约预付/充值 → Gate hook 检查付费状态 → 返回 true/false
- 用户体验从"付费即跳"变为"先充值，再跳"（可通过 SSU dApp 缓解）
...
- Smart Gate hook 调用 TollRegistry 验证付费状态（跨合约读取）
...
- 90 秒 Demo：运营者设置通行费 → 玩家通过 SSU dApp 预付 → Gate hook 验证放行 → 星门跳转
...
> **可行性**：⚠️ 取决于燃料代币属性 — 需确认 Fuel 是否为标准 `Coin<T>` 类型（可通过官方 Discord 或 world contracts 源码验证）

**核心概念**：专为 EVE Frontier 燃料设计的 AMM 交易所，基于恒积公式（x·y=k），支持 Fuel/SUI 双向兑换。

**前提条件**：
- 如果 Fuel 是标准 `Coin<T>` → ✅ 直接建 AMM 池
- 如果 Fuel 绑定到特定 Object → 🟡 需包装层（Wrapper Coin）
- 如果 Fuel 是纯游戏内数值 → 🔴 不可行，需转为 EVE/SUI 交易对或部落代币 DEX
...
### 4. AutoMod Factory / Assembly 配置平台

> **可行性**：🟡 可行但需重新设计 — Move 不支持从合约内动态发布新 Move 包（无 Solidity 的 CREATE2 等价物）

**核心概念**：让非开发者玩家通过"填参数"而非"写代码"配置自定义 Smart Assembly 行为，类似 Shopify 模板市场。

**技术限制与可行方案**：
- ❌ ~~动态部署新 Move 合约~~ — Move 不支持运行时代码生成
- ✅ **方案 A（推荐）**：预部署参数化合约，用户只调函数传参数来个性化行为
- ✅ **方案 B**：Web 前端收集参数 → 后端生成 Move 代码 → 用户签名发布（类似 Remix IDE）
...
**Sui 集成方式**：
- Move 合约：预部署可配置模块 + Dynamic Fields 存储用户参数
- Walrus：模板源码/文档存储（内容引用 blob ID）
- zkLogin：Web2 友好登录（注：CCP 已集成，此处为第三方 dApp 场景）
...
> **可行性**：🟡 技术可行但有 ToS 风险 — 自动化操作可能被视为 botting，需与 CCP 确认合规性

**核心概念**：AI 代理自动管理玩家的 Network Node（补充燃料、监控状态、触发告警、执行策略）。

**风险提示**：
- ⚠️ EVE Online 历史上严厉打击 bot，EVE Frontier 可能沿用类似政策
- ⚠️ CCP 只为玩家直接操作代付 Gas，Agent 需自行承担 Gas 费用
- 需在 Discord 或官方渠道确认自动化 Agent 是否被允许
...
**Sui 集成**：AgentPermission Object（授权代理操作）· Walrus 策略历史记录 · Sponsored Tx（需自行代付）
...
### ~~8. AccessPass / zkLogin 身份门控~~ — 🔴 建议砍掉

> **可行性**：🔴 与官方功能高度重叠 — CCP 已将 zkLogin 集成到 EVE Frontier 玩家登录流程，玩家已可通过 Google/Apple 无摩擦登录

**冲突分析**：
- CCP 已实现"Web2 玩家无摩擦进入"的目标
- 再做一层 zkLogin 身份门控 = 重复造轮子
- 唯一不冲突场景：作为第三方 dApp 的身份验证服务（类似 OAuth provider），但与 EVE 生态关系弱

**原评分**：~~77/100~~ → **建议移除出优先列表**

---

### 9. FrontierData / 游戏数据分析平台

> **可行性**：🟡 可行但与官方 API 部分重叠 — 官方 World API (Swagger) 已提供角色/部署/库存/世界状态查询

**核心概念**：在官方 World API 之上构建数据分析层——聚合分析、历史趋势、PvP 统计、价格预测，类似 evepraisal 之于 ESI。

**差异化定位**：
- 官方 API = 原始数据查询（实时状态）
- FrontierData = 加工后的分析数据（价格趋势、活跃度热力图、PvP 统计、联盟实力评估）

**赚钱机制**：
- 免费版 100 次/天 · 基础版 $9/月 · 专业版 $29/月无限制
- 先例：evepraisal 年收入估算 $30K+

**Sui 集成**：Walrus 历史数据存档 · Sui Pay 链上订阅支付
...
> **可行性**：⚠️ 同 FuelDEX，依赖 Fuel 是否为 `Coin<T>` + 冷启动需双边流动性
...
### ~~20. FuelHedge / 燃料价格期权~~ — 🔴 建议砍掉

> **可行性**：🔴 当前阶段不可行 — 需要预言机（不存在）+ 交易市场（未建立）+ 足够流动性（玩家基数不够），至少需 FuelDEX + FrontierOracle 先运行 3-6 个月

**核心概念**：玩家可买入燃料价格看涨/看跌期权，对冲运营成本。

**赚钱机制**：期权费收入 · 做市商价差

**原评分**：~~67/100~~ → **建议移除出优先列表**

---

## 完整排名（含可行性评估）

> 可行性基于 EVE Frontier Sui 迁移后实际开放功能评估，详见 [FEASIBILITY.md](./FEASIBILITY.md)

| 排名 | 项目 | 得分 | 可行性 | 关键风险 | 仓库 |
|------|------|------|--------|---------|------|
| 🥇 1 | GateMarket — 星门通行费交易所 | **88** | 🟡 需调整 | Gate hook 只返回 bool，需两步收费 | [p1-gatemarket](https://github.com/autoteam-claw/p1-gatemarket) |
| 🥈 2 | TrustEscrow — 玩家去中心化托管 | **85** | 🟢 可行 | 无 | [p2-trustescrow](https://github.com/autoteam-claw/p2-trustescrow) |
| 🥉 3 | FuelDEX — 燃料 AMM 交易所 | **83** | ⚠️ 待确认 | Fuel 是否为 `Coin<T>` | [p3-fueldex](https://github.com/autoteam-claw/p3-fueldex) |
| 4 | AutoMod Factory — Assembly 配置平台 | **82** | 🟡 需调整 | Move 不支持动态部署，改为参数化配置 | [p4-automod-factory](https://github.com/autoteam-claw/p4-automod-factory) |
| 5 | ChainRep — 链上声誉系统 | **80** | 🟢 可行 | 无 | [p5-chainrep](https://github.com/autoteam-claw/p5-chainrep) |
| 6 | FrontierOracle — 价格预言机 | **80** | 🟡 冷启动难 | 需先有交易市场才有数据可聚合 | — |
| 7 | FrontierAgent — AI 运营代理 | **78** | 🟡 ToS 风险 | 可能被视为 botting | — |
| 8 | ~~AccessPass — zkLogin 身份门控~~ | ~~77~~ | 🔴 冲突 | **CCP 已集成 zkLogin，建议砍掉** | — |
| 9 | FrontierData — 数据分析平台 | **77** | 🟡 部分重叠 | 官方 World API 已提供基础查询 | — |
| 10 | FrontierInsurance — 链上游戏保险 | **76** | 🟡 高复杂度 | 精算模型 + 历史数据不足 | — |
| 11 | StorageMarket — 仓储单元租赁 | **75** | 🟢 可行 | 无 | — |
| 12 | FrontierChat — Seal 加密通讯 | **75** | 🟡 需求存疑 | 玩家已用 Discord，迁移意愿低 | — |
| 13 | SpaceYield — 燃料质押生息 | **72** | ⚠️ 待确认 | 同 FuelDEX 前提 + 冷启动难 | — |
| 14 | TribalVault — 联盟链上金库 | **72** | 🟢 可行 | 无 | — |
| 15 | KillMint — 链上战报 NFT | **72** | 🟢 可行 | 无（KillMail 链上可读已确认） | — |
| 16 | frontier-flow++ — 可视化合约编排 | **70** | 🟡 需调整 | 做 IDE 工具而非链上产品 | — |
| 17 | ReputationBounty — 链上赏金平台 | **70** | 🟢 可行 | KillMail 目标识别信息完整度待确认 | — |
| 18 | FrontierLend — 资产抵押借贷 | **69** | 🟡 依赖多 | 预言机→估值→清算链条过长 | — |
| 19 | AllianceIndex — 联盟排行榜 | **68** | 🟢 可行 | 无 | — |
| 20 | ~~FuelHedge — 燃料价格期权~~ | ~~67~~ | 🔴 不可行 | **基础设施全部缺失，建议砍掉** | — |
...
| 赛道 | 派出项目 | 可行性 | 状态 | 获奖期望值 | 备注 |
|------|---------|--------|------|-----------|------|
| **Utility** | GateMarket | 🟡 需调整两步收费 | ✅ 已完成 | $5,000 × 85% = $4,250 | Demo 叙事需更新 |
| **Technical** | AutoMod Factory | 🟡 改为配置平台 | 合约完成 | $5,000 × 75% = $3,750 | 重新定义核心概念 |
| **Creative** | FrontierAgent | 🟡 需确认 ToS | 未开始 | $5,000 × 60% = $3,000 | 降级：ToS 风险 |
| **Weirdest** | KillMint | 🟢 无阻塞 | 未开始 | $5,000 × 75% = $3,750 | 最高 ROI 新项目 |
| **Live Integration** | StorageMarket | 🟢 无阻塞 | 未开始 | $5,000 × 60% = $3,000 | A 类形态 |
| **Overall 1st** | GateMarket | 🟡 | ✅ 已完成 | $15,000 × 30% = $4,500 | 两步收费略降体验分 |

**修正后总期望收益**：保守估计 $10,000–15,000（考虑可行性调整和赛道唯一获奖约束）

### ② 立即行动优先级（可行性修正后）

1. **GateMarket** → 已完成，**但需调整为两步收费模式**并更新 Demo 叙事，然后部署到 Stillness
2. **KillMint** → 🟢 无阻塞，1-2 天开发，ROI 最高的新项目（锁定 Weirdest）
3. **StorageMarket** → 🟢 无阻塞，开发量小，A 类形态（锁定 Live Integration）
4. **AutoMod Factory** → 🟡 需先将核心概念从"模板部署"改为"参数化配置"，再补全前端
5. **FrontierAgent** → 🟡 **先在 Discord 确认自动化 Agent 合规性**，再决定是否投入开发

### ②-b 需要立即执行的验证任务

| 验证项 | 影响范围 | 验证方式 | 优先级 |
|--------|---------|---------|--------|
| Fuel 是否为 `Coin<T>` | FuelDEX、SpaceYield、FuelHedge | 读 world contracts 源码或问官方 Discord | P0 |
| 自动化 Agent 是否违反 ToS | FrontierAgent | 问官方 Discord | P1 |
| KillMail 事件数据完整度 | KillMint、ReputationBounty | 读 Primordium 索引器或链上事件 | P1 |
...
- 控制交通 + 控制能源 + 控制定价权
- 任何新进入者都要依赖你的基础设施
- ⚠️ FuelDEX 和 FrontierOracle 可行性分别依赖 Fuel 代币属性和链上交易量

---

## Phase 6.5：可行性评估摘要

> 基于 EVE Frontier Cycle 5（Sui 迁移后）实际开放功能的技术验证，详见 [FEASIBILITY.md](./FEASIBILITY.md)

### 三个必须注意的技术事实

| # | 事实 | 影响 |
|---|------|------|
| 1 | **Smart Gate `canJump()` 只返回 bool** | GateMarket 不能"付费即跳"，需两步收费（预付→验证） |
| 2 | **Move 不支持动态代码部署** | AutoMod Factory 不能从合约内发布新 Move 包，需改为预部署参数化配置 |
| 3 | **zkLogin 已被 CCP 官方集成** | AccessPass 与官方功能完全重叠，建议砍掉 |

### 20 项目可行性分布

```
🟢 可行（7 个）    TrustEscrow · ChainRep · StorageMarket · TribalVault · KillMint · ReputationBounty · AllianceIndex
🟡 需调整（9 个）  GateMarket · AutoMod · FrontierOracle · FrontierAgent · FrontierData · FrontierInsurance · FrontierChat · frontier-flow++ · FrontierLend
⚠️ 待确认（2 个）  FuelDEX · SpaceYield（依赖 Fuel 代币类型）
🔴 砍掉（2 个）    AccessPass（官方已做）· FuelHedge（基础设施缺失）
```
...
| 优先级 | 项目 | 可行性 | 开发量 | 预期价值 |
|--------|------|--------|--------|---------|
| **P0** | KillMint | 🟢 | 1-2 天 | Weirdest 赛道 $5K |
| **P0** | GateMarket（调整两步收费） | 🟡 | 已完成，改 Demo | Utility + Overall |
| **P1** | StorageMarket | 🟢 | 3-5 天 | Live Integration $5K |
| **P1** | TrustEscrow | 🟢 | 已完成 | Utility 备选 |
| **P2** | ChainRep（补前端） | 🟢 | 合约完成 | Technical 竞争 |
| **P2** | AutoMod Factory（重新定义） | 🟡 | 需改设计 | Technical 竞争 |
| **P3** | ReputationBounty | 🟢 | 3-5 天 | Weirdest 备选 |
| **验证后决定** | FuelDEX | ⚠️ | 需确认 Fuel 类型 | DeFi 核心 |
| **验证后决定** | FrontierAgent | 🟡 | 需确认 ToS | Creative 赛道 |

---

## Phase 7：Bootcamp 创意库 ROI 精选

> 来源：[hoh-zone/eve-bootcamp](https://github.com/hoh-zone/eve-bootcamp) 创意库（[Sui 核心特性 100 例](https://github.com/hoh-zone/eve-bootcamp/tree/main/src/idea) + [常规综合 100 例](https://github.com/hoh-zone/eve-bootcamp/tree/main/src/idea_general)），筛除与上方 20 项目重叠的创意后，按纯商业 ROI 排名。

### 评分模型（商业 ROI 导向）

| 维度 | 权重 | 说明 |
|------|------|------|
| 💰 持续收益能力 | 35% | 收入飞轮清晰度、被动收入占比 |
| 🎯 市场需求真实度 | 25% | 玩家是否有真实痛点、愿意付费 |
| ⛓️ Sui 差异化 | 20% | 是否利用 Sui 独有能力（PTB/Kiosk/Seal/Object） |
| ⚡ Solo 实现速度 | 20% | 单人 2 周内能否出 MVP |

### 排名

| 排名 | 项目 | 来源 | 收益 | 需求 | 差异化 | 速度 | 得分 | 核心卖点 |
|------|------|------|------|------|--------|------|------|---------|
| 1 | **蓝图母版与版税工厂** | Sui #095 | 9 | 8 | 8 | 7 | **82** | 原创者铸造不可变蓝图母版，制造商购买限次生产许可，每次生产自动返版税，形成设计者→生产商→分销商永久分成链 |
| 2 | **垄断式战争武装专营店** | Sui #051 | 8 | 7 | 9 | 8 | **80** | 顶级火炮只能从 Kiosk 买，Transfer Policy 强制每次转手抽版税，原制造者永久获利 |
| 3 | **太空闪电贷** | Sui #001 | 8 | 7 | 10 | 6 | **78** | PTB + Hot Potato 原子级闪电贷——借飞船→采矿→卖→还船，一笔 tx 完成，出借人零风险 |
| 4 | **太空运单撮合市场** | General #009 | 8 | 9 | 6 | 7 | **77** | 太空版滴滴货运——发货人锁押金，司机接单运输，物品到达目标储物箱后链上自动结算 |
| 5 | **密封盲拍拍卖行** | Sui #055 | 8 | 7 | 9 | 6 | **76** | Seal 加密出价 + Kiosk 暗拍稀缺物品（星门执照等），竞拍者互不知底牌，价高者得 |
| 6 | **情报黑市付费墙** | General #015 | 7 | 8 | 8 | 7 | **75** | 侦察兵将敌方舰队坐标加密为 Dynamic Field，其他指挥官付 SUI 解锁查看，Seal 天然适配 |
| 7 | **炮塔限时租赁协议** | General #011 | 7 | 8 | 7 | 8 | **75** | 时间戳临时授权 OwnerCap 给雇佣兵，24h 后自动失效，完全去中心化的防御力出租 |
| 8 | **联盟战利品均分系统** | General #013 | 6 | 9 | 6 | 9 | **74** | 战利品扔进分账合约，瞬间按参战名单均分到所有队员钱包，1-2% 服务费 |
| 9 | **去中心化联盟股份化** | General #051 | 9 | 7 | 7 | 5 | **73** | 联盟变上市公司——发行股票 Token，链上公开产能数据不可造假，股东按利润分红 |
| 10 | **开源舰队 AI 策略仓库** | Sui #086 | 8 | 7 | 8 | 5 | **72** | 炮塔 AI、物流调度、价格模型做成可购买/订阅/fork 的策略插件市场，Walrus 存储 |
| 11 | **深空加密巨幅广告牌** | General #042 | 7 | 6 | 7 | 9 | **72** | 繁忙航线节点挂 Display 标准广告牌，按天拍卖像素绘制权，太空版 Reddit Place |
| 12 | **全服倒计时争夺战** | Sui #094 | 7 | 8 | 7 | 6 | **71** | 链上 Clock 驱动的公开倒计时争夺赛——资源箱/限时星门/悬赏池归零瞬间自动结算 |
...
现有生态                     Bootcamp 新增
─────────                    ─────────────
GateMarket ←── 通行数据 ──→ 情报黑市付费墙（Seal 加密情报）
FuelDEX ←── 燃料定价 ──→ 太空闪电贷（借船→采矿→卖燃料→还船）
TrustEscrow ←── 信任层 ──→ 太空运单撮合（物流托管结算）
ChainRep ←── 声誉 ──→ 炮塔租赁（按声誉定租金）
AutoMod Factory ←── 模板 ──→ AI 策略仓库（策略即模板）
KillMint ←── 战报 ──→ 联盟战利品均分（战后自动分赃）
TribalVault ←── 金库 ──→ 联盟股份化（金库→上市公司）
FrontierOracle ←── 价格 ──→ 蓝图版税工厂（按市价抽版税）
```
...
| 优先级 | 项目 | 理由 | 预计开发 |
|--------|------|------|---------|
| **P0** | 蓝图母版与版税工厂 | 收入最持续（每次生产永久抽成），与 Kiosk 深度集成，生态杠杆最大 | 7-10 天 |
| **P1** | 垄断式武装专营店 | Kiosk 代码可与蓝图工厂共用 80%，两个项目一起做成本最低 | +3 天（共用基础） |
| **P2** | 联盟战利品均分系统 | 开发量最小（1-2 天），痛点最真实，快速上线验证市场 | 1-2 天 |
| **P3** | 炮塔限时租赁 | 简单时间锁逻辑，与 StorageMarket 共用租赁框架 | 2-3 天 |
| **P4** | 太空闪电贷 | Sui 差异化最强（PTB + Hot Potato），Demo 效果炸裂，但实现复杂 | 10-14 天 |

> **快速赚钱路径**：先做 P2（战利品均分，1-2 天上线收手续费）→ 再做 P0+P1（蓝图+武装店，Kiosk 套件共享代码）→ P3（租赁）→ P4（闪电贷做技术 showcase）

---

## 技术栈

- **链上**：Sui Move（EVE Frontier Smart Object Framework 扩展模式）
- **游戏框架**：SOF（Smart Object Framework）— Sui 上替代 EVM 的 MUD 框架
- **前端**：Next.js 14 (App Router) + Tailwind CSS
- **数据库**：PostgreSQL + Prisma ORM
- **钱包**：@mysten/dapp-kit
- **数据存储**：Walrus（链上历史数据）
- **访问控制**：Seal（私密数据门控）
- **部署环境**：Nova（沙箱测试）→ Stillness（生产服务器）

## 开发进度

| 项目 | Move 合约 | Next.js 全栈 | 可行性 | 状态 | 下一步 |
|------|-----------|-------------|--------|------|--------|
| P1 GateMarket | ✅ 完整 | ✅ 完整 | 🟡 需调整 | 完成 | 改为两步收费模式 + 更新 Demo |
| P2 TrustEscrow | ✅ 完整 | ✅ 完整 | 🟢 可行 | 完成 | 部署到 Stillness |
| P3 FuelDEX | ✅ 完整 | 🔄 开发中 | ⚠️ 待确认 | 暂停 | 先确认 Fuel 代币类型 |
| P4 AutoMod Factory | ✅ 完整 | 🔄 开发中 | 🟡 需调整 | 进行中 | 重新定义为配置平台 |
| P5 ChainRep | ✅ 完整 | 🔄 开发中 | 🟢 可行 | 进行中 | 补全前端 |
